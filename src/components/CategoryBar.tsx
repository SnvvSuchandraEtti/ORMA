...
export default function CategoryBar() {
  ...
  const handleCategoryClick = (slug: string) => {
    if (isDragging) return // Prevent click if the user was dragging
  ...
  const onMouseUp = () => {
    // Only clear dragging state after a short delay so click handler can read it
    setTimeout(() => setIsDragging(false), 50)
  }

  const onMouseMove = (e: React.MouseEvent) => {
    if (!scrollRef.current) return
    // If buttons aren't pressed, we're not dragging
    if (e.buttons !== 1) {
      setIsDragging(false)
      return
    }
  ...
  return (
    <div className="fixed left-0 right-0 z-30 bg-white/95 dark:bg-[#000000]/95 backdrop-blur-md border-b border-black/[0.08] dark:border-white/5 transition-all duration-300" style={{ top: '52px' }}>
      <nav aria-label="Categories" className="max-w-[1760px] mx-auto px-4 md:px-6 lg:px-10 relative">
        ...
        <div
          ref={scrollRef}
          className={`flex items-end gap-6 overflow-x-auto hide-scrollbar py-3 ${isDragging ? 'cursor-grabbing select-none' : 'cursor-grab'}`}
          role="tablist"
